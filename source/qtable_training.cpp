...
float calculate_reward(float fragmentation, float latency_ns, bool compaction_used)
{
    ...
    if (fragmentation <= 0.01f)
        reward += (float)REWARD_BIG;
    else if (fragmentation <= 0.02f)
        reward += (float)REWARD_MEDIUM;
    else if (fragmentation <= 0.05f)
        reward -= (float)PUNISHMENT_SMALL;
    else
        reward -= (float)PUNISHMENT_BIG;

    if (latency_ns < 15.0f)
        reward += (float)REWARD_BIG;
    else if (latency_ns <= 30.0f)
        reward += (float)REWARD_MEDIUM;
    else if (latency_ns <= 60.0f)
        reward -= (float)PUNISHMENT_MEDIUM;
    else
        reward -= (float)PUNISHMENT_BIG;
...
void simulate_action(int action, float *reward, int *next_state)
{
    ...
    float old_latency = sim_state.latency_ns;
...
void train_q_table()
{
    ...
    float epsilon_decay = g_epsilon_decay;
    float epsilon_min = g_epsilon_min;
    ...
    for (int episode = 0; episode < g_total_episode; episode++)
    {
        ...
        for (int step = 0; step < step_per_episode; step++)
        {
            ...
            float reward;
            int next_state;
            simulate_action(action, &reward, &next_state);
